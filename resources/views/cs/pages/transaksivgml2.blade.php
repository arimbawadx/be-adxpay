...
@section('title') adx-pay | Transaksi Mobile Legend @endsection

@section('content-header') Transaksi Mobile Legend @endsection
  ...
  <div class="container">
    <div class="row">
      <div class="col-md-6 col-12">
        <form class="card card-body" id="form-confirm" method="post" action="/cs/transaksi/vgml/3">
          {{csrf_field()}}
            <div class="form-group">
                <label for="no_pelanggan">ID</label>
                <input id="no_pelanggan" required="" autocomplete="off" type="text" class="form-control" name="no_pelanggan" placeholder="Masukan ID ML">
            </div>
          <div class="form-group">
            ...
            <select id="produk" name="produk" required class="form-control">
              ...
              @if($pp->status=="ACTIVE")
              <option value="{{$pp->product_id}}">{{$pp->operator_name}} {{$pp->product_detail}} | Harga Rp. {{number_format($pp->product_price+2000)}}</option>
              @endif
              @endforeach
            </select>
          </div>
