...
@section('title') adx-pay | Transaksi Transfer Bank @endsection

@section('content-header') Transaksi Transfer Bank @endsection
  ...
  <div class="container">
    <div class="row">
      <div class="col-md-6 col-12">
        <form class="card card-body" id="form-confirm" method="post" action="/cus/transaksi/Bank/3">
          {{csrf_field()}}
          ...
          <div class="form-group">
            ...
            <select id="produk" name="produk" required class="form-control">
              ...
              @if($pp->status=="ACTIVE")
              @if($pp->product_id != "CEK".$pp->operator_id)
              <option value="{{$pp->product_id}}">{{$pp->operator_name}} {{$pp->product_name}} | Harga Rp. {{number_format($pp->product_price+4500)}}</option>
              @endif
              @endif
              @endforeach
            </select>
