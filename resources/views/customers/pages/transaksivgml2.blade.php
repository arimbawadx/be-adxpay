...
@section('title') adx-pay | Transaksi Diamond ML @endsection

@section('content-header') Transaksi Diamond ML @endsection
  ...
  <div class="container">
    <div class="row">
      <div class="col-md-6 col-12">
        <form class="card card-body" id="form-confirm" method="post" action="/cus/transaksi/vgml/3">
          {{csrf_field()}}
            <div class="form-group">
                <input name="operatorProduk" value="{{$idProduk}}" type="hidden">
                <label for="no_hp">ID @if(session()->get('dataLoginCustomers')['verified']==0) <span class="badge badge-danger">Saldo : Rp. {{number_format($saldo->saldo, 0, '', '.')}}</span> @endif</label>
                <input id="no_hp" required="" autocomplete="off" type="text" class="form-control" name="no_hp" placeholder="Masukan ID ML">
            </div>
          <div class="form-group">
            ...
            <select id="produk" name="produk" required class="form-control">
              ...
              @if($pp->status=="ACTIVE")
              <option value="{{$pp->product_id}}">{{$pp->product_name}} {{$pp->operator_name}} | Harga Rp. {{number_format($pp->product_price+2000)}}</option>
              @endif
              @endforeach
            </select>
          </div>
          ...
          @endif
          <button id="btn-trx-confirm-vg" type="button" class="btn btn-primary">Kirim</button>
        </form>
      </div>
      <div class="col-md-6 col-12">
        <div class="card">
